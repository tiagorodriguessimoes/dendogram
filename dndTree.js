...
function prep_data_to_export(data) {
	// It removes useless information for the user;
	function remove_uncessary_fields(d) {
		delete d.x;
		delete d.x0;
		delete d.y;
		delete d.y0;
	...
	// It goes through data
	function dfs(data) {
		remove_uncessary_fields(data);
		if (data.length >= 1) {
			for (var i = 0; i < data.length; i++) {
				var el = data[i];
				remove_uncessary_fields(el);
				dfs(el);
			}
		}
	...
	dfs(data);
	// decycle removes the loops from JSONs. Mandatory here.
	return JSON.decycle(data);
};

function export_data() {
	var data = prep_data_to_export(tree_root);
	return JSON.stringify(data);
};


function export_click(el) {
